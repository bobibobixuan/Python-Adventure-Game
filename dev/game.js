// 当前选中的单元
        ...
        async function verifyDevPass() {
            ...
            // 1. 将用户输入的密码转换为字节数据并进行 SHA-256摘要加密
            const msgUint8 = new TextEncoder().encode(input);
            const hashBuffer = await crypto.subtle.digest('SHA-256', msgUint8);

            // 2. 将加密后的 Buffer 数据转换为 16 进制的字符串
            const hashArray = Array.from(new Uint8Array(hashBuffer));
            const inputHash = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');

            // 3. 目标密文 (对应明文密码的 SHA-256 Hex 值)
            const expectedHash = "e18e010158ed9479ce5b953795a789a79ba6599b1b6e70528ed7b262f8681aa0";

            // 4. 对比密文
            if (inputHash === expectedHash) {
                closeDevMenu();
                switchScreen('adminScreen');
                updateAdminStatus('✓ 验证成功');
            } else {
                document.getElementById('devPassInput').value = '';
                updateAdminStatus('密码错误');
            }
        }
        ...
        function adminUnlockAll() {
            gameState.unitLevelUnlocked[gameState.currentUnit] = [true, true, true, true, true, true];
            saveGameState();
            renderUnits();
            renderLevelMap();
            updateAdminStatus('✓ 已解锁当前单元全部关卡');
        }

        function adminLockAll() {
            gameState.unitLevelUnlocked[gameState.currentUnit] = [true, false, false, false, false, false];
            gameState.unitLevelStars[gameState.currentUnit] = [0, 0, 0, 0, 0, 0];
            saveGameState();
            renderUnits();
            renderLevelMap();
            updateAdminStatus('✓ 已重置当前单元进度');
        }

        function adminUnlockAchievements() {
            gameState.achievements = achievementList.map(a => a.id);
            saveGameState();
            updateAdminStatus('✓ 成就已解锁');
        }

        function adminClearAchievements() {
            gameState.achievements = [];
            saveGameState();
            updateAdminStatus('✓ 成就已清除');
        }

        function adminSetMaxStars() {
            gameState.unitLevelStars[gameState.currentUnit] = [3, 3, 3, 3, 3, 3];
            gameState.unitLevelUnlocked[gameState.currentUnit] = [true, true, true, true, true, true];
            saveGameState();
            renderUnits();
            renderLevelMap();
            updateAdminStatus('✓ 当前单元已设置满星');
        }

        function adminClearWrongs() {
            gameState.wrongQuestions = [];
            saveGameState();
            updateAdminStatus('✓ 错题已清空');
        }
